    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-zinc-500">
          审核用户提交的新模型申请。通过后会创建 status=observing、pinned=false 的本地模型。
        </p>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 align-top text-xs text-zinc-600 dark:text-zinc-400">
                      ...
                      {r.releasedAt && <div>发布日期：{r.releasedAt}</div>}
                    </td>
                    <td className="px-4 py-3 align-top">
...
function formatWeight(value: boolean | null): string {
  if (value === null) return "未知";
  return value ? "开源权重" : "闭源权重";
}

function formatMeta(row: Awaited<ReturnType<typeof listModelRequestsForAdmin>>[number]): string {
  const parts = [
    row.contextTokens !== null ? `上下文 ${row.contextTokens.toLocaleString()}` : null,
    row.outputTokens !== null ? `最大输出 ${row.outputTokens.toLocaleString()}` : null,
    row.inputModalities?.length ? `输入类型 ${row.inputModalities.join(",")}` : null,
    row.outputModalities?.length ? `输出类型 ${row.outputModalities.join(",")}` : null,
    row.supportsReasoning ? "支持推理" : null,
    row.supportsToolCall ? "支持工具调用" : null,
  ].filter(Boolean);
